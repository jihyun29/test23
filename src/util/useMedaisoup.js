...
//   // 1번 스트림 시작- if 로 발언자(isteller)만 실행되게 로직 구현 이미 발언자로직 구분됨
...
//   // 4. 서버 프로듀서쪽 transport만들고 클라이언트로 params를 보냄
...
//   // button 6. consumer transport생성- 여기서 배심원고민해야함
...
// // export { getLocalStream, localVideoRef, remoteVideoRef };

// 이렇게 함수 단위별로는 사용가능
export const streamSuccess = async (stream) => {
  console.log("스트림?????????????????", stream);
  localVideoRef.current.srcObject = stream;

  //미디어 스트림에서 오디오 및 비디오 트랙 가져오기

  const audioParams = { track: stream.getAudioTracks()[0], ...audioParams };
  const videoParams = { track: stream.getVideoTracks()[0], ...videoParams };
